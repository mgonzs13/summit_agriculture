...
class GpsWpCommander(Node):
    ...
    def start_wpf(self):
        """
        Function to start the waypoint following in a continuous loop
        """
        ...
        while rclpy.ok():
            ...
            for wp in wps:
                ...
                log = "x={:f}, y={:f}, z={:f}".format(
                    self.resp.pose.position.x,
                    self.resp.pose.position.y,
                    self.resp.pose.position.z,
                )
                self.get_logger().info(log)
            ...
            self.get_logger().info("Ruta completada. Reiniciando...")
